...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === "squad") {
      ...
      const relegatedClubs = req.body.relegatedClubs || null;
      validatedAnswers = answers.map(a => {
        ...
        const meetsGoals = match.goals >= threshold;
        // If relegated clubs filter is set, check the player played for one of them
        let meetsClub = true;
        if (relegatedClubs && match.team) {
          meetsClub = relegatedClubs.some(rc => match.team.toLowerCase().includes(rc.toLowerCase().split(" ")[0]) || rc.toLowerCase().includes(match.team.toLowerCase().split(" ")[0]));
        }
        const valid = meetsGoals && meetsClub;
        return {
          player: a.player, answer: a.answer,
          identified_as: match.name, valid,
          reason: valid
            ? `${match.name} scored ${match.goals} goals for ${match.team} in ${season}`
            : !meetsClub ? `${match.name} played for ${match.team}, not one of the relegated clubs in ${season}`
      ...
      for (const ans of unique) {
        ...
        try {
          // Search across multiple seasons to find players who appeared at any point since 2010
      let results = await apiFetch(`/players?search=${encodeURIComponent(ans)}&league=${LEAGUE_ID}&season=2023`, apiKey);
      if (!results || results.length === 0) results = await apiFetch(`/players?search=${encodeURIComponent(ans)}&league=${LEAGUE_ID}&season=2015`, apiKey);
      if (!results || results.length === 0) results = await apiFetch(`/players?search=${encodeURIComponent(ans)}&league=${LEAGUE_ID}&season=2012`, apiKey);
    ...
    } else if (type === "table_lookup") {
    // 0 API calls — pure lookup against provided valid answers
    const validAnswers = (req.body.validAnswers || []).map(a => a.toLowerCase());
    validatedAnswers = answers.map(a => {
      const t = norm(a.answer);
      const match = (req.body.validAnswers || []).find(v => norm(v) === t || v.toLowerCase().includes(t) || t.includes(norm(v)));
      return {
        player: a.player, answer: a.answer,
        identified_as: match || a.answer,
        valid: !!match,
        reason: match ? `${match} is correct` : `${a.answer} is not one of the valid answers`
      };
    });
